...
function refreshHtmlContent(content) {
		...
		content.browseCnt = content.browseCnt ? content.browseCnt+"人阅读" : '';
		...
		document.querySelector('.dz-app-article-related').innerHTML = '<span>' + content.author + '</span><span>'+content.translator+'</span>';
		document.querySelector('.dz-app-read').innerHTML = '<span>' + content.publishTime + '</span><span>' + content.readTime + '</span><span>' + content.browseCnt + '</span>';
		var _container = document.querySelector('.dz-app-content'),
			_p = _container.querySelectorAll('p');
